  ...
  socket.onmessage = function (e) {
    ...
    if (data.type == "typing") {
      ...
      if (usersTyping.length == 1) {
        ...
        string =
          usersTyping.join(", ") + " and " + lastUser + " are writing...";
      ...
      switch (data.info) {
        ...
        case "update":
          showChat(
            "info",
            null,
            data.user.oldun + " changed its name to " + data.user.un
          );
        ...
        case "connection":
          ...
          showChat(
            "info",
            null,
            data.user.un + userip + " connected to the server"
          );

          clients[data.user.id] = data.user;
          document.getElementById("users").innerHTML =
            Object.keys(clients).length + " USERS";
          break;
        ...
        case "disconnection":
          ...
          if (data.user.un != null) {
            showChat(
              "info",
              null,
              data.user.un + userip + " disconnected from the server"
            );
          }
        ...
        case "spam":
          showChat(
            "global",
            null,
            "You have to wait 1 second between messages. Continuing on spamming the servers may get you banned. Warning " +
              data.warn +
              " of 5"
          );
          break;
    ...
    if (m) {
      m.forEach(function (e, i, a) {
        ...
        if (e.indexOf("//gfycat") !== -1) {
          var oldUrl = e;
          e = e
            .replace("//gfycat.com", "//gfycat.com/cajax/get")
            .replace("http://", "https://");

          $.getJSON(e, function (data) {
            testImage(
              data.gfyItem.gifUrl.replace("http://", "https://"),
              mid,
              oldUrl
            );
          });
      ...
      switch (command[0].toLowerCase()) {
        ...
        case "em":
          if (value.substring(command[0].length).length > 1) {
            if (
              (command[0] == "pm" || command[0] == "msg") &&
              value.substring(command[0].concat(command[1]).length).length > 2
            ) {
              sendSocket(
                value.substring(command[0].concat(command[1]).length + 2),
                "pm",
                command[1],
                "PM"
              );
            ...
            if (command[0] == "alert") {
              sendSocket(
                value.substring(command[0].length + 2),
                "global",
                null,
                username
              );
            ...
            if (
              command[0] == "role" &&
              value.substring(command[0].concat(command[1]).length).length > 3
            ) {
              sendSocket(
                command[1],
                "role",
                value.substring(command[0].concat(command[1]).length + 3)
              );
            } else if (command[0] == "role") {
              showChat(
                "light",
                "Error",
                "Use /" + command[0] + " [user] [0-3]"
              );
            }
        ...
        default:
          showChat(
            "light",
            "Error",
            "Unknown command, use /help to get a list of the available commands"
          );
          break;
      }
    } else {
